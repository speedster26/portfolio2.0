...
const OneSkill = ({ directionLeft , skill }: Props) => {
  return (
    <motion.div initial={{x:directionLeft?-75:75,opacity:0}} transition={{duration:1.5}} whileInView={{opacity:1,x:0}} className='group relative flex cursor-pointer'>
        <Image width={64} height={64} className="rounded-full border border-gray-500 object-contain p-2 object-center w-16 h-16 xl:w-32 xl:h-32 filter group-hover:grayscale transition duration-300 ease-in-outs " src={urlFor(skill?.icon).url()} alt={skill.title} />
