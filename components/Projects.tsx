    ...
    <div className='relative w-full flex overflow-x-scroll overflow-y-hidden snap-x z-20 scrollbar-thin scrollbar-track-gray-400/20 scrollbar-thumb-[#F7AB0A]/80'>
        ...
        {projects.map((project,i) => (
            <div key={project} className='w-screen flex-shrink-0 snap-center flex flex-col space-y-5 items-center justify-center p-20 md:p-44 h-screen'>
                <motion.img initial={{y:-200,opacity:0}} transition={{duration:1.2}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className='max-w-xs' src={"https://d2slcw3kip6qmk.cloudfront.net/marketing/blog/2017Q2/project-planning-header@2x.png"} alt="" />
                <motion.div initial={{y:300,opacity:0}} transition={{duration:1.2}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className='space-y-5 md:space-y-10 px-0 md:p-10 max-w-6xl'>
                    ...
                    <p className='text-lg md:text-left'>
                        Lorem ipsum dolor sit amet consectetur adipisicing elit. Delectus, eaque. Minus laboriosam repellendus enim nulla dicta voluptate sit, et nisi ab amet ipsa error dolor
                    </p>
                </motion.div>
            </div>
        ))}
