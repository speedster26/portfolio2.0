...
const Projects = ({projects}: Props) => {
  ...
  return (
    ...
    <div className='relative w-full flex overflow-x-scroll overflow-y-hidden snap-x z-20 scrollbar-thin scrollbar-track-gray-400/20 scrollbar-thumb-[#31e390]/80'>
        {/* ProjectsCard */}
            ...
            <div key={project._id} className='w-screen flex-shrink-0 snap-center flex flex-col space-y-5 items-center justify-center p-20 md:p-44 h-screen'>
                ...
                <motion.div initial={{y:300,opacity:0}} transition={{duration:1.2}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className='space-y-5 md:space-y-10 px-0 md:p-10 max-w-6xl'>
                    <h4 className='text-2xl md:text-4xl font-semibold text-center'>
                        <span className='underline decoration-[#31e390]/50'>
                    ...
                    <div className='flex items-center justify-center space-x-2'>
                        {project.technologies.map((tech)=>(
                            <Image key={tech._id} width={40} height={40} className='h-10' src={urlFor(tech.icon).url()} alt={""}/>
                        ))}
                    </div>
    ...
    <div className='w-full absolute top-[30%] bg-[#31e390]/10 left-0 h-[500px] -skew-y-12'/>    
    </motion.div>
  )
}
